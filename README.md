...
<html lang="en">
 ...
 <body>
  ...
  <script>
    ...
    // Initialize app
    function initApp() {
      // Load activities from localStorage for GitHub deployment
      ...
      app.innerHTML = `
        <div class="w-full min-h-screen flex items-center justify-center p-6">
          <div class="w-full max-w-md">
            <div class="fade-in rounded-2xl shadow-2xl p-8 bg-white">
              <div class="text-center mb-8">
                <div class="text-6xl mb-4">🧠</div>
                <h1 class="text-4xl font-bold mb-2 text-gray-800">${config.platformName}</h1>
                <p class="text-lg text-gray-600">${config.tagline}</p>
              </div>
              ...
              <form id="loginForm" onsubmit="event.preventDefault(); handleLogin();">
                <div class="mb-4">
                  ...
                  <input 
                    ...
                    id="username" 
                    required 
                    class="w-full px-4 py-3 rounded-lg border-2 border-gray-200 focus:outline-none focus:border-purple-500 transition-colors"
                ...
                <div id="loginMessage" class="mb-4 p-3 rounded-lg text-center hidden"></div>
              ...
              <p class="text-center mt-6 text-sm text-gray-500">Demo: Use any username/password to continue</p>
            </div>
          </div>
        </div>
      `;
      ...
      app.innerHTML = `
        <div class="w-full min-h-screen">
          ...
          <div class="max-w-7xl mx-auto p-6 md:p-8">
            <div class="mb-12">
              <h2 class="text-2xl font-bold mb-6 text-gray-800">Choose Your Subject</h2>
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ${Object.entries(subjects).map(([key, subject]) => `
                  <div 
                    onclick="selectSubject('${key}')" 
                    class="subject-card rounded-xl p-6 shadow-lg card-hover bg-white"
                  >
                    <div class="text-5xl mb-4">${subject.icon}</div>
                    <h3 class="text-xl font-bold mb-2 text-gray-800">${subject.name}</h3>
                    <p class="text-gray-600">${subject.topics.length} topics available</p>
                  </div>
                `).join('')}
              </div>
            </div>
                    ...
                    <div class="flex items-center justify-between py-4 border-b border-gray-100 last:border-0">
                      <div class="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p class="text-sm text-gray-500">${subjects[activity.subject]?.name || activity.subject}</p>
                        </div>
                      </div>
                      <div class="text-right">
                        ${activity.completed 
                          ? `<span class="px-3 py-1 rounded-full text-white bg-green-500 text-sm font-medium">✓ Completed</span>` 
                          : `<span class="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-sm font-medium">In Progress</span>`
                        }
                        ${activity.score ? `<p class="text-sm text-gray-600 mt-1">Score: ${activity.score}%</p>` : ''}
                      </div>
                    </div>
                  `).join('')}
                </div>
            ...
            <div>
              <h2 class="text-2xl font-bold mb-6 text-gray-800">Your Progress</h2>
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="rounded-xl shadow-lg p-6 bg-white">
                  <div class="flex items-center justify-between mb-3">
                    <p class="font-semibold text-gray-800">Activities Completed</p>
                ...
                <div class="rounded-xl shadow-lg p-6 bg-white">
                  <div class="flex items-center justify-between mb-3">
                    <p class="font-semibold text-gray-800">Total Activities</p>
    ...
    function renderSubjectView() {
      ...
      app.innerHTML = `
        <div class="w-full min-h-screen">
          ...
          <div class="max-w-7xl mx-auto p-6 md:p-8">
            <div class="mb-8">
              <label for="classSelect" class="block text-xl font-semibold mb-3 text-gray-800">Select Your Class</label>
              <select 
                id="classSelect" 
                onchange="selectClass(this.value)" 
                class="px-4 py-3 rounded-lg border-2 border-gray-200 focus:outline-none focus:border-purple-500 bg-white text-gray-800 min-w-64"
              >
            ...
            <div id="topicsSection" class="hidden">
              <div class="mb-12">
                <h2 class="text-2xl font-bold mb-6 text-gray-800">Core Topics</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ${subject.topics.map((topic, idx) => `
                    <div 
                      class="rounded-xl p-5 shadow-lg card-hover cursor-pointer bg-white" 
                    ...
                    >
                      <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                          ...
                          <div>
                            <h3 class="font-semibold text-gray-800 text-lg">${topic}</h3>
                            <p class="text-sm text-gray-500">Interactive lesson</p>
                          </div>
                        </div>
              ...
              <div class="mb-12">
                <h2 class="text-2xl font-bold mb-6 text-gray-800">${config.worksheetLabel}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                          ...
                          <div>
                            <h3 class="font-semibold text-gray-800 text-lg">${topic} Practice</h3>
                            <p class="text-sm text-gray-500">10 practice problems</p>
                          </div>
                        </div>
                        <div class="text-2xl">→</div>
                      </div>
              ...
              <div>
                <h2 class="text-2xl font-bold mb-6 text-gray-800">${config.assignmentLabel}</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                          ...
                          <div>
                            <h3 class="font-semibold text-gray-800 text-lg">Assignment ${idx + 1}: ${topic}</h3>
    ...
    function showMessage(text, type) {
      ...
      messageDiv.className = 'fixed top-6 right-6 px-6 py-4 rounded-lg shadow-xl fade-in z-50 text-white font-medium';
 ...
 <script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'9a4e4b2da73ca734',t:'MTc2NDIxMTU3MS4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
</html>
